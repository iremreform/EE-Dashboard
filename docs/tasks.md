...
- [x] Preserve a seed-free, security-hardened production baseline in `supabase/migrations`; keep demo seed data out of production migrations.
...
- [x] Review live Supabase RLS/database grants, function/default privileges, schema access, Storage bucket privacy/policies, Auth configuration, SSL enforcement, and database network restrictions.
- [ ] Confirm the current one-hour staging signed-read expiry or shorten it after real 4K playback testing.
...
- [x] Add initial automated unit coverage for report parsing/validation, media constraints, request safety, and pickup issue detection.
- [ ] Add integration coverage for authorization, form persistence, reservation matching, driver ownership, and status/audit behavior.
